...
@section('content')
    <div class="max-w-3xl mx-auto py-10 px-4">
        <div class="bg-white p-8 shadow-lg rounded-xl border border-gray-200">
            ...
            <h2 class="text-2xl font-bold mb-6 text-gray-800">Edit Variabel</h2>

            <form action="{{ route('variabel.update', $variabel) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="kode_variabel"
                        ...
                        value="{{ old('kode_variabel', $variabel->kode_variabel) }}">
                    @error('kode_variabel')
                ...
                <div>
                    ...
                    <input type="text" name="nama_variabel"
                        class="w-full px-2 py-1 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200"
                        value="{{ old('nama_variabel', $variabel->nama_variabel) }}">
                    @error('nama_variabel')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="urutan"
                        class="w-full px-2 py-1 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200"
                        value="{{ old('urutan', $variabel->urutan) }}">
                    @error('urutan')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
